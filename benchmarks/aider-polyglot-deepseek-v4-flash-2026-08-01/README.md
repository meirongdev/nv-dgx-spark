...
> 同一个模型此前在 SWE-bench Lite 上只跑出 14.3%（20 条冒烟，单轮 oracle 设置），
> 且 **19/20 条预测的 unified diff hunk 头行数算错**（声称 8 行上下文只给 4 行），
> 其中 12 条只是恰好被 harness 的 `patch --fuzz=5` 兜底救回。
> 这到底是「模型不会做结构化编辑」，还是「模型不会手写 diff 时自己算行号」？

**答案：是后者。** 换成不需要算行号的编辑格式后，格式合规率 100%、通过率 82–88%。

> 那次 SWE-bench 的工作区与归档已于 2026-08-02 清理（结论已被本文推翻，
> 且 ARM64 上跑它需要 12 处 harness 补丁、代价过高）。上面的数字保留在此作为
> 本次实验的动机记录；完整原始记录见 git 历史 `394ffe8^`。
...
>
> **⚠️ 2026-08-05 更正**：上面这个前提是错的。当时引用的 "~50 聚合" 低报了约 3.7 倍，
> 实测 c1/c2/c4/c6 聚合 = 67/113/143/**186** tok/s，6 路并发是单流的 **2.8 倍**。
> 也就是说这 4.5 小时里并发确实在起作用，"基本等于串行时间"的说法不成立；
> 未来同类跑分可以放心靠并发提速（上限仍是 `max_num_seqs=6`）。
> 见 `benchmarks/bench-full-2026-08-05/`。
