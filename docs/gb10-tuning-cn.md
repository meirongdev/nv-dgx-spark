...
make clock-cap-verify     # ← 唯一可靠判据,见下(⚠️ 它自己曾静默失效过,见 §3.1)
...
### 3.1 ⚠️ 这个"唯一判据"自己静默失效过(2026-09-02 → 09-03)

**换主力栈之后，`clock-cap-verify` 有约 24 小时什么都没在验。**

`verify` 需要发一条真实生成才能采到负载期频率，而它把 model 名写死成了
`deepseek-v4-flash`。2026-09-02 主力栈换成 Flash-Next 后:

1. 服务端返回 **400**(model 不存在);
2. 但 `curl` 的退出码仍是 **0** —— 旧版脚本据此认为生成成功;
3. 于是它拿引擎**空载**期的采样算出判据行并照常打印。

致命之处在于**空载频率本来就贴住上限**，所以输出与"锁生效"长得完全一样，
只有采样点数(`n=2` vs `n≈20`)透露了异常 —— 而判据行里并不提 n:

```
  head/rank0: max=2177 mean=2177 n=2      ← 空载。看起来完全正常
  → 判据:负载期 mean/max 若明显低于 ~2400 且贴住你设的上限,锁生效。
```

**已修**(2026-09-03):

- model 名提成 `CAP_MODEL`，默认跟随当前主力栈;
- 判据不再看 `curl` 退出码，改为解析响应里的 `completion_tokens`，
  **必须 ≥50 才继续**;
- 生成失败时打印服务端响应前 200 字节并 `exit 3`，**绝不**再打印一个可能被
  读成"通过"的数字。修的时候发现同一个洞还有第二层：远端脚本已经 `exit 3` 了，
  外层函数仍无条件把判据行 echo 出来 —— 现在判据行只在远端成功时才打印，
  失败时改印「本次 verify **没有结论**」。

双向回归（2026-09-03 实测）：

| 用例 | 退出码 | 输出 |
|---|---|---|
| `make clock-cap-verify`（正确 model） | 0 | 生成 300 token，n=18，判据行照常 |
| `CAP_MODEL=deepseek-v4-flash …`（错的 model） | **3** | 打印 404 响应 + 「没有结论」，**无判据行** |

> 负向用例和正向用例一样重要 —— 这个 bug 的全部危害就在于**失败路径看起来像
> 成功**，只测正向路径永远发现不了。

修复后带真实负载复验(Flash-Next 在跑):

| | 结果 |
|---|---|
| 负载 | 生成 300 token ✅ |
| head/rank0 | mean **2171**(n=20) |
| peer/rank1 | mean **2190**(n=20) |

→ 锁在两台上都仍然生效，与 2026-08-25 的落地记录一致。

**教训已归纳进 `docs/gotchas-cn.md` #9 和 `docs/stack-switch-cn.md`:**
一个分不清"通过"和"根本没运行"的检查器，比没有检查器更糟 —— 后者你知道自己
没验，前者让你以为验过了。
