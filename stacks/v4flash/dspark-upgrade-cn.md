...
> 供任何机器/会话复现。基础栈背景见 `stacks/v4flash/runbook-cn.md`。
>
...
>    `make stop STACK=v4flash` / `make run STACK=v4flash`**(2026-08-13 起跑在 k3s 上)。
>    **镜像重编后多一步必做**:两台各自
...
make stop STACK=v4flash        # 2026-08-13 前是 sudo systemctl stop deepseek-v4-flash
...
# 7) 起服务 + 验证 → 停机窗口结束
make run STACK=v4flash && make status STACK=v4flash && make test STACK=v4flash
```
...
- `make test STACK=v4flash`:单流编码 tok/s —— **第一次请求 ~41 tok/s(看起来没提速),
...
7. **⚠️ 任何 `build-and-copy.sh`/`docker build` 之前必须先停生产栈**(2026-07-04 事故):
   ...
   **不管构建看起来多"轻",动手前一律 `make stop STACK=v4flash`。**
...
`systemctl stop/start deepseek-v4-flash` 换成 `make stop STACK=v4flash` / `make run STACK=v4flash`。
**新增一步必做**:镜像重编后要在两台各自
