...
> DSpark 调优细节见 `stacks/v4flash/dspark-upgrade-cn.md`。
...
- **必须用 jasl/vllm fork**:`codex/ds4-sm120-min-enable` 带 GB10 sm_121 支持 + **triton 稀疏 MLA**(`VLLM_TRITON_MLA_SPARSE=1`,绕开 FlashMLA)。
  ...
  **jasl fork 现在唯一不可替代的价值是 DSpark**,详见 `stacks/v4flash/dspark-upgrade-cn.md`。
...
vLLM flag 的**唯一真相源**是 `stacks/v4flash/recipe.yaml`;线上实际执行的启动
命令是它的渲染产物 `stacks/v4flash/k8s/configmap-launch.yaml`(两者要一起改)。关键 flag:
...
`stacks/v4flash/dspark-upgrade-cn.md`——那一次的数直接丢掉重测。
